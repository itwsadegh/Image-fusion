function J = imfuse( para, varargin )
%imfuse function makes images which are taken with different depth and a focus image will be generated.

global g_para;
set_parameters( para );
g_para.nImg = length( varargin ); % number of images
nImg = g_para.nImg;
nLv = g_para.nLv; % number of decomposition
...
% Converting RGB images into the YCrCb color. 
...
% Create the weight of the approximation coefficient sub-band 
sy = S(1,1);  sx = S(1,2);
...
W_sum = sum( Wp,  3 );
Mask  = W_sum ~= 0;
W_sum = W_sum + ~Mask; % interpolate 1 to avoid show 'divide by 0'
...
% Deblur the approximation sub-band image
...
% This function gives the size of output image obtained from the specified decomopsition level. 
